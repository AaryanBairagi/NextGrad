...
async function predict() {
    try {
        ...
        if (result.error) {
            ...
            updateSuccessRate(result.chance);  // New function to update the success rate text
        }
    } catch (error) {
...
// New function to update success rate text dynamically
function updateSuccessRate(chance) {
    const successRateElement = document.getElementById('rateValue');
    const successTextElement = document.getElementById('successRate');

    successRateElement.textContent = `${Math.round(chance)}%`;

    if (chance >= 50) {
        successRateElement.style.color = '#2563eb';  // Blue for success
        successTextElement.style.color = '#2563eb';
    } else {
        successRateElement.style.color = '#ffffff';  // White for failure
        successTextElement.style.color = '#ffffff';
    }
}
